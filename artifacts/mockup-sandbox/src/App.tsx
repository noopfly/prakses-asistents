...
function Home() {
  ...
  return <main id="main">
    ...
    <section className="section functionality" id="funkcionalitate">
      ...
      <div className={`functionality-impact functionality-impact--${role}`}><div className="impact-copy"><p className="eyebrow">Mūsu ietekme</p><h2>{role==="gp"?"Savlaicīgāks kontakts ar pacientiem.":"Būtiskais redzams pirms konsultācijas."}</h2><p>{role==="gp"?"Prakses Asistenta atgādinājumi palīdz palielināt pacientu atsaucību uz vēža skrīninga aicinājumiem un mērķtiecīgāk organizēt profilakses darbu.":"Strukturēts pacienta pārskats vienuviet parāda analīžu dinamiku, aktuālo terapiju un svarīgāko aprūpes kontekstu."}</p></div>{role==="gp"?<div className="bars"><h3>Vēža skrīninga atsaucība (%)</h3>{[["Krūts",50,81],["Dzemdes kakla",43,88],["Zarnu",20,45],["Prostatas",40,65]].map(([l,a,b])=><div key={String(l)}><span>{l}</span><i style={{width:`${a}%`}}></i><i className="after" style={{width:`${b}%`}}></i><b>{b}%</b></div>)}</div>:<div className="impact-points"><article><BarChart3/><h3>Analīžu dinamika</h3><p>Izmaiņas pārskatāmas vienā skatā.</p></article><article><ClipboardList/><h3>Aktuālā terapija</h3><p>Medikamenti un būtiskais konteksts vienuviet.</p></article><article><HeartPulse/><h3>Pacienta kopaina</h3><p>Skaidrāks pamats sarunai konsultācijā.</p></article></div>}</div>
    </section>
